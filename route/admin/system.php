...
Route::group(function () {
    ...
    Route::group('platform_merchant', function () {
        Route::get('lst', '/lst')->name('systemPlatformMerchant')->option([
            '_alias' => '商户号列表'
        ]);
        Route::post('save/:id', '/save')->name('systemSavePlatformMerchant')->option([
            '_alias' => '修改商户号'
        ]);
        Route::post('create', '/create')->name('systemCreatePlatformMerchant')->option([
            '_alias' => '新增商户号'
        ]);
        Route::delete('del/:id/:merchantId', '/del')->name('systemDelPlatformMerchant')->option([
            '_alias' => '删除商户号'
        ]);

        Route::get('form', '/form')->name('systemGetForm')->option([
            '_alias' => '获取设置商户号from表单',
            '_auth' => false,
        ]);
        Route::get('form/:id', '/form')->name('systemGetForm')->option([
            '_alias' => '获取设置商户号from表单',
            '_auth' => false,
        ]);

        Route::get('getPlatformMerchantConfig', '/getPlatformMerchantConfig')->name('systemGetPlatformMerchantConfig')->option([
            '_alias' => '获取商户号用途列表'
        ]);
        Route::post('savePlatformMerchantConfig', '/savePlatformMerchantConfig')->name('systemSavePlatformMerchantConfig')->option([
            '_alias' => '修改商户号用途'
        ]);
    })->prefix('admin.system.merchant.platform_merchant')->option([
        '_auth' => true,
    ]);
    ...
    Route::group('profitSharing', function () {
        Route::get('show', '/getProfitSharingSetting')->name('systemConfigConfigValueShow')->option([
            '_alias' => '读取分销配置',
        ]);
        Route::post('set', '/setProfitSharingSetting')->name('systemConfigConfigValueShow')->option([
            '_alias' => '设置分销配置',
        ]);
    })->prefix('admin.system.config.ConfigValue')->option([
        //'_path' => '/maintain/dataBackup',
        '_auth' => true,
    ]);
