...
if (!function_exists('getAppidAndAppSecret')) {
    /**
     * 用appid查appSecret
     *
     * @return void
     */
    function getAppidAndAppSecret($appid = ''): array
    {
        // 若不指定appid,则从请求参数中取
        if (empty($appid)) {
            $appid = request()->appid();
        ...
        $key = sprintf(WxAppletModel::CACHE_APPID_TO_SECRET, $appid);
        $appSecret = $redis->get($key);
        if ($appSecret) {
            return [$appid, $appSecret];
        } else {
            $appSecret = app()->make(WxAppletRepository::class)->getAppSecretByAppid($appid);
            if ($appSecret) {
                $redis->set($key, $appSecret, ["NX", "EX" => 3600]);
            }
        }
        return [$appid, $appSecret];
    }
...
if (!function_exists('getCreateCouponMerchant')) {
    /**
     * 获取建券商户
     *
     ...
     * @return int|mixed
     */
    function getCreateCouponMerchant(string $stockId = '')
    {
        if (empty($stockId)) {
            // 随机获取一个制券商户
        ...
        /**
         * @var \app\common\repositories\system\merchant\PlatformMerchantRepository $make
         */
        $make = app()->make( \app\common\repositories\system\merchant\PlatformMerchantRepository::class);
        $fields = 'merchant_id as pay_routine_mchid, key as pay_routine_key, cert_path as pay_routine_client_cert, key_path as pay_routine_client_key';
        $where = [
            'merchant_id' => $mchId,
            'is_del'      => \app\common\model\system\merchant\PlatformMerchant::DELETED_NO,
        ];
        $merchant = $make->getOne(0, $where, $fields);
        return method_exists($merchant, 'toArray') ? $merchant->toArray() : [];
    }
}

if (!function_exists('getSendCouponMerchant')) {
    /**
     * 获取发券商户
     *
     * @param string $stockId
     ...
     * @return array
     */
    function getSendCouponMerchant(string $stockId = '', string $couponCode = '')
    {
        if (empty($stockId)) {
            // 随机获取一个制券商户
...
if (!function_exists('mini_unique_id')) {
    function mini_unique_id(int $length = 16)
    {
...
if (!function_exists('sendRequest')) {
    ...
    function sendRequest($method, $url, $param)
    {
        ...
        Log::debug("guzzle:request:params:$method:$url". json_encode($param));
        try {
            $client = new Client();
            $response = $client->request($method, $url, $param);
            $result = json_decode($response->getBody()->getContents(), true);
            Log::debug("guzzle:request:result-". json_encode($result, JSON_UNESCAPED_UNICODE));
            return $result;
        } catch (\Exception $exception) {
            Log::error("guzzle:request:error" . $exception->getMessage());
            return [];
        }
    }

    if (!function_exists('sendAsyncRequest')) {
        /**
         * 发送asyncRequest请求
         * @param $method
         * @param $url
         ...
         * @date    2023/3/9 14:25
         */
        function sendAsyncRequest($method, $url, $body)
        {
            if(is_array($body)){
                $body = json_encode($body, JSON_UNESCAPED_UNICODE);
            }
            Log::debug("guzzle:async_request:params:$method:$url-". $body);
            try {
                $client = new Client();
                $headers = [
                ...
                Log::debug("guzzle:async_request:result-". json_encode($result, JSON_UNESCAPED_UNICODE));
                return $result;
            } catch (\Exception $exception) {
                Log::error("guzzle:async_request:error" . $exception->getMessage());
                return [];
            }
        }
         ...
         */
        if (!function_exists('get_preview_code')) {

            function get_preview_code($id, $appid, $page, $data)
            {
                $res =  app()->make(RoutineQrcodeRepository::class)->getPreviewCode($page, $data, '280', $appid);
                $namePath = 'preview_' . $id.'.jpg';
                $uploadType = (int)systemConfig('upload_type') ?: 1;
                $upload = UploadService::create($uploadType);
                ...
                return tidy_url($imageInfo['dir'], 0);
            }
        }
        ...
        if (!function_exists('get_order_status')) {
            function get_order_status($data): string
            {
                if ($data['is_del'] != 0) {
                    return '已关闭';
                }
                if ($data['paid'] == 0) {
                    return '待付款';
                } elseif ($data['order_type'] == 0 || $data['order_type'] == 2) {
                    return StoreOrder::ORDER_STATUS[$data['status']];
                } else {
                    return StoreOrder::STATUS_MAP[$data['status']];
                }
            }
        }
...
if (!function_exists('sendMessageToWorkBot')) {
    /**
     * 发送企微群告警信息
     * @param array $params
     * @author  wzq
    ...
    function sendMessageToWorkBot(array $params)
    {
        ...
        $message = json_encode($params , 256);
        $message = strlen($message) > 2000 ? substr($message , 0 ,2048) : $message;
        $msg = $params['msg'] ?? '未知异常';
        ...
        sendAsyncRequest('post', config('app.qw_robot'), $body);
    }
}
