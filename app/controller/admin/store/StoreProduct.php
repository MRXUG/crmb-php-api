...
class StoreProduct extends BaseController
{
     ...
     * @param  \app\common\repositories\system\merchant\MerchantAdRepository  $merchantAdRepository
     */
    public function __construct(App $app, repository $repository, MerchantAdRepository $merchantAdRepository)
    {
        parent::__construct($app);
        $this->repository = $repository;
        $this->merchantAdRepository = $merchantAdRepository;
    }
    ...
    {
        $params = $this->request->param(['appid']);

        return app('json')->success($this->repository->qrcode($id, $params['appid']));
    }
     ...
     * @date    2023/3/7 21:06
     */
    public function adLst($id)
    {
        [$page, $limit] = $this->getPage();
        $where['goods_id'] = $id;
        $data = $this->merchantAdRepository->getList($where, $page, $limit);
        return app('json')->success($data);
    }
