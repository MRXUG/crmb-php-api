...
class Config extends BaseController
{
    ...
    public function uploadCert($field, $merId = 0)
    {
        $file = $this->request->file($field);
        validate([
            "file|文件" => ['fileExt' => 'pem'],
        ],[
            'file.fileExt' => '请上传pem格式文件',
        ])->check(['file' => $file]);
        if (!$file) return app('json')->fail('请上传附件');
        $saveName = Filesystem::disk('certs')->putFile( $merId, $file);
        // $upload = UploadService::create(1);
        // $data = $upload->to(app()->getRootPath() . 'resources/certs/' . $merId)->validate()->move($field);
        if (!$saveName) {
            return app('json')->fail("文件上传失败");
        }
        return app('json')->success(['src' => path_to_url($saveName)]);
    }
     ...
     * @date    2023/3/9 16:22
     */
    public function uploadLicense($field)
    {
        $file = $this->request->file($field);
        validate([
            "file|文件" => ['fileExt' => 'png,jpg', "fileSize" => 8*1024*1024],
        ],[
            'file.fileExt' => '请上传png、jpg格式图片',
            'file.fileSize' => '请上传大小在8M以内的图片'
        ])->check(['file' => $file]);
    ...
    public function uploadAsName()
    {
        ...
        if (!$file) return app('json')->fail('请上传附件');
        $res = Filesystem::putFileAs('', $file, $file->getOriginalName());
