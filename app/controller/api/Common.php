...
class Common extends BaseController
{
     ...
     * 商家券领券回调
     *
     * 1638941761 -> https://qlpf-dev.vchangyi.com/api/notice/receive-coupon-notify?key=MTYzODk0MTc2MQ==
     * 1620183676 -> https://qlpf-dev.vchangyi.com/api/notice/receive-coupon-notify?key=MTYyMDE4MzY3Ng==
     *
     */
    public function merchantCouponNotify()
    {
        // TODO 验签 请求头 wechatpay-signature
        $raw = request()->post();
        Log::info('商家券领券回调：' . json_encode($raw));

        $header = is_array(request()->header()) ? json_encode(request()->header()) : request()->header();
        Log::info('商家券领券回调：' . $header);

        $extra = [
            'mch_id' => base64_decode(request()->get('key')),
        ];

        $response = MerchantCouponService::create(MerchantCouponService::CALLBACK_COUPON, $extra)->handleNotify($raw);
    ...
    public function routineNotify($merId)
    {
        try {
            return response(MiniProgramService::create($merId)->handleNotify()->getContent());
        } catch (Exception $e) {
            Log::info('支付回调失败:' . var_export([$e->getMessage(), $e->getFile() . ':' . $e->getLine()], true));
        }
    }
