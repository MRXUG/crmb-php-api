...
//        }
    ...
    {
        $user = $this->request->userInfo()->hidden([
            'label_id',
            'group_id',
            'pwd',
            'addres',
    ...
    public function auth()
    {
        ...
        $oauth->setRequest(new Request($request->get(), $request->post(), [], [], [], $request->server(),
            $request->getContent()));
        try {
    ...
    public function mpAuth()
    {
        ...
        if ($code && !$userInfoCong) {
            try {
                ...
                return app('json')->fail('获取session_key失败，请检查您的配置！',
    ...
    public function verify(UserAuthValidate $validate)
    {
        ...
        if (!in_array($data['phone'], ['18710839146','13484926808'])) {
            try {
                $sms_code = str_pad(random_int(1, 9999), 4, 0, STR_PAD_LEFT);
                $sms_time = systemConfig('sms_time') ? systemConfig('sms_time') : 30;
                SmsService::create()->send($data['phone'], 'VERIFICATION_CODE',
                    ['code' => $sms_code, 'time' => $sms_time]);
            } catch (Exception $e) {
                return app('json')->fail($e->getMessage());
            }
        } else {
...
//                    $userInfoCong = $miniProgramService->getUserInfo($code);
            ...
            if ($auth['type'] === 'wechat') {
                ...
                if ($auth['type'] === 'app_wechat') {
                    $oauth = WechatService::create()->getApplication()->oauth;
                    try {
                        $wechatInfo = $oauth->user(new AccessToken([
                    ...
                    $user = app()->make(WechatUserRepository::class)->syncAppUser($wechatInfo['unionid'], $wechatInfo,
                        'App', $createUser);
                    if (!$user) {
                        throw new ValidateException('授权失败');
                    }
    ...
    public function cancel()
    {
        ...
        $order = app()->make(StoreOrderRepository::class)->search([
            'uid' => $user['uid'],
            'paid' => 1
        ])->where('StoreOrder.status', 0)->count();
...
//        }
    ...
    public function thirdpartyCode2Session(){
        ...
        if (!isset($data['unionid']))return app('json')->status(400,$data['errmsg']);
