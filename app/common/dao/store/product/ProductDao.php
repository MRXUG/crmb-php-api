...
//        }
        ...
        $query->Join('StoreSpu U', 'Product.product_id = U.product_id')
            ...
            ->when((isset($where['MerStatus']) && $where['MerStatus'] !== ''), function ($query) use ($where) {
                $query->where('M.status', $where['MerStatus']);
            })
     ...
     */
    public function getAll(int $page = 1, int $limit = 10, int $productId = 0)
    {
