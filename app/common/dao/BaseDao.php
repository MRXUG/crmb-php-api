...
abstract class BaseDao
{
    ...
    public function deleteByIsDel(int $id)
    {
        return ($this->getModel())::getDB()->where($this->getPk(), $id)->update(['is_del' => 1]);
    }
    ...
    public function findOrCreateByData(array $where , array $data)
    {
        $res = ($this->getModel()::getDB())->where($where)->find();
        if(!$res)$res = $this->getModel()::create($data);
        return $res;
    }
     ...
     * @throws DataNotFoundException
     */
    public function createOrUpdate(array $where, $data)
    {
        $res = ($this->getModel()::getDB())->where($where)->find();
        if(!$res) {
            ($this->getModel())::getDB()->save($data);
        } else {
            ($this->getModel())::getDB()->where($where)->save($data);
        }
        return $res;
    }
    ...
    {
        return ($this->getModel()::getDB())->where($where)->value($field);
    }
}
