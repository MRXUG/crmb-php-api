...
class MerchantAuthMiddleware extends BaseMiddleware
{
    ...
    public function before(Request $request)
    {
        ...
        \think\facade\Log::info("admin_before_log:" . json_encode([
                'admin' => $admin->toArray(),
        ...
        if ($admin->level) {
            ...
            $menus = count($rules) ? ($merchant->type_id ? $menu->typesByRoutes($merchant->type_id,
                $rules) : $menu->idsByRoutes($rules)) : [];
            $msg = '没有权限访问';
        } else {
            $rules = [];
        ...
        $request->macro('checkAuth', function ($name, $vars) use (&$merchant, &$admin, &$menus, &$menu) {
            if (!$name || (!$admin->level && !$merchant->type_id)) {
                return true;
            }
            $isset = false;
