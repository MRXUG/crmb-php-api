...
{
    /**
     * @var RelatedBusinessDao
     */
    private $relatedBusinessDao;

    /**
     * MerchantRepository constructor.
     * @param MerchantDao $dao
     * @param RelatedBusinessDao $relatedBusinessDao
     */
    public function __construct(MerchantDao $dao, RelatedBusinessDao $relatedBusinessDao)
    {
        $this->dao = $dao;
        $this->relatedBusinessDao = $relatedBusinessDao;
    }
    ...
    {
        $fields = 'sort, mer_id, mer_name, real_name, mer_phone, mer_address, mark, status, create_time,is_best,is_trader,
            type_id,category_id,copy_product_num,export_dump_num,is_margin,margin,mer_avatar';
        ...
        foreach ($list as &$item) {
            $merchantId = $item['mer_id'];
            $item['mchid'] = merchantConfig($merchantId, 'pay_routine_mchid');
        }
    ...
    public function form(?int $id = null, array $formData = [])
    {
        ...
        $rule = [
            Elm::input('mer_name', '店铺名称')->required(),
            ...
            Elm::input('real_name', '商户主体')->required()->placeholder('请输入企业工商注册名称'),
            /*Elm::frameImage('business_license', '上传营业执照',  config('admin.admin_prefix') . '/setting/uploadPicture?field=business_license&type=1')
                ->value($formData['business_license'] ?? '')->required()
                ->modal(['modal' => false])
                ->width('896px')
                ->height('480px'),*/
            Elm::uploadImage('business_license', '上传营业执照', rtrim(systemConfig('site_url'), '/') . Route::buildUrl('configUploadLicense', ['field' => 'file'])->build())->headers(['X-Token' => request()->token()])->required(),
...
//                $item['recommend'] = isset($where['delivery_way']) ? $item['CityRecommend'] : $item['AllRecommend'];
    ...
    public function infoFrom($id, $data)
    {
        $form = Elm::createForm(Route::buildUrl('systemSaveMerchantPayInfo', ['id' => $id])->build());
        $form->setRule([
            Elm::input('pay_routine_mchid', '微信商户号', $data['pay_routine_mchid']??'')->required(),
            Elm::input('pay_routine_key', 'v2key', $data['pay_routine_key']??'')->required(),
            Elm::input('pay_routine_v3_key', 'v3key', $data['pay_routine_v3_key']??'')->required(),
            Elm::input('pay_routine_serial_no', '证书序列号', $data['pay_routine_serial_no']??'')->required(),
            Elm::uploadFile(
            ...
            Elm::uploadFile('pay_routine_client_key', 'apiclient_key.pem', rtrim(systemConfig('site_url'), '/') . Route::buildUrl('configUploadCert', [
                    'field' => 'file',
                    'merId' => $id,
                ])->build())->headers(['X-Token' => request()->token()])->required(),
        ]);
        return $form->formData($data)->setTitle('设置商家微信支付商户号');
    }
     ...
     * @date    2023/3/11 9:41
     */
    public function correlationForm(int $id = 0): Form
    {
        $formData = [];
        if ($id > 0) {
            $formData = $this->relatedBusinessDao->get($id)->toArray();
            $form = Elm::createForm(Route::buildUrl('saveCorrelation', ['id' => $id])->build());
        } else {
            $form = Elm::createForm(Route::buildUrl('createCorrelation')->build());
        }

        $form->setRule([
            Elm::input('name', '关联主体企业名称', $formData['name'] ?? '')->required()->placeholder('请输入企业工商注册名称'),
            Elm::uploadImage(
                'business_license',
                '上传营业执照',
                rtrim(systemConfig('site_url'), '/') . Route::buildUrl('merConfigUploadLicense', ['field' => 'file']
                )->build()
            )->headers(['X-Token' => request()->token()])->required(),
        ]);

        return $form->formData($formData)->setTitle($id > 0 ? '编辑店铺关联主体' : '添加店铺关联主体');
    }
