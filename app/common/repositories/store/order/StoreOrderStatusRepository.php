...
class StoreOrderStatusRepository extends BaseRepository
{
    ...
    */
    ...
    public function selectOrders15DaysAfterReceive($orderId = [])
    {
        $dateBefore15Days = date('Y-m-d H:i:s', time() - 15 * 86400);
        return $this->dao->query([
            'change_type' => self::ORDER_STATUS_TAKE
        ])->whereIn('order_id', $orderId)
            ->whereTime('change_time', '<=', $dateBefore15Days)
            ->column('order_id');
    }
}
