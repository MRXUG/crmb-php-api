...
class Product extends BaseModel
{
    ...
    {
        if (!$this || !$this->is_show || !$this->is_used || !$this->status || $this->is_del || !$this->mer_status) {
            return false;
        }
    ...
    public function getSeckillStatusAttr()
    {
        ...
        $end_day   = strtotime($this->seckillActive['end_day']);
        if (!$this->seckillActive) {
            return '';
        }

        if ($this->seckillActive['status'] !== -1) {
            //还未开始
            if ($start_day > time() || $this->is_show !== 1) {
                return 0;
            }
            ...
            if ($start_day <= $day && $day <= $end_day) {
                //未开始
                if ($this->seckillActive['start_time'] > $_h) {
                    return 0;
                }

                //已结束
                if ($this->seckillActive['end_time'] <= $_h) {
                    return -1;
                }

                //进行中
                if ($this->seckillActive['start_time'] <= $_h && $this->seckillActive['end_time'] > $_h) {
                    return 1;
                }
