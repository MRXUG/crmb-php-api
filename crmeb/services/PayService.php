...
use app\common\repositories\wechat\UserOpenidRelationRepository;
...
//        Log::info("支付的类型 " . $method . json_encode($user));
    ...
    public function payRoutine(User $user)
    {
        ...
        $userOpenidRelationRep = app()->make(UserOpenidRelationRepository::class);
        [$appid, $appSecret] = getAppidAndAppSecret();
        $relationInfo = $userOpenidRelationRep->routineOpenIdByWhere(['wechat_user_id' => $user->wechat_user_id , 'appid' => $appid]);
